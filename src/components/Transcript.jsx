...
const Transcript = ({ transcript, segments, currentTime, setCurrentTime, ...props }) => {
    ...
    const readableTranscript = useMemo(() => {
        ...
        for (const segment of segments) {
            ...
            // If the speaker is the same, append to their text
            if (label === currentSpeaker) {
                currentSpeakerText += ` ${segmentWords.join(' ')}`;
            } else {
                // If the speaker changes, push previous speaker's text and reset
                if (currentSpeaker !== null && currentSpeakerText.trim().length > 0) {
                    result.push(`${currentSpeaker}: ${currentSpeakerText.trim()}`);
                }
    ...
    const downloadTextTranscript = () => {
        const blob = new Blob([readableTranscript], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
    ...
    return (<>
        ...
        <div className="flex justify-center border-t text-sm text-gray-600 max-h-[150px] overflow-y-auto p-2 scrollbar-thin">
            <button
                className="flex items-center border px-2 py-1 rounded-lg bg-green-400 text-white hover:bg-green-500"
                onClick={downloadTextTranscript}
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 mr-1">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                </svg>
